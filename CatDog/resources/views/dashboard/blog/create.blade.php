...
@section('title')
    Blog /Create/D A S h T R A P
@endsection


@section('content')

<x-breadcum catdog="Blog's Create Page"></x-breadcum>
            ...
            <div class="card-body">
                ...
                <form role="form" action="{{ route('blog.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3"  style="padding-left: 35%">
                        <div class="col-sm-9">
                            <img id="catDog" src="{{ asset('upload/default/CatDog.jpg') }}" style="width:100%; height:100%;">
                        </div>
                    </div>
                    <div class="row mb-2">
                        <label for="inputPassword5" class="col-sm-3 col-form-label">Blog Thumbnail</label>
                        <div class="col-sm-9">
                            <input type="file" onchange="document.querySelector('#catDog').src = window.URL.createObjectURL(this.files[0])" class="form-control @error('thumbnail') is-invalid @enderror" id="inputPassword5" name="thumbnail">
                        </div>
                        @error('thumbnail')
                        <p class="text-danger">{{ $message }}</p>
